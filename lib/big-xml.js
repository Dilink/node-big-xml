...
function BigXmlReader(stream, recordRegEx, options) {
  ...
  self.once('error-record-process', function(e) {
    if (stream.response) { // it's the stream from request package
      stream.response.destroy(e);
    }
    else {
      stream.destroy(e); // it's a nodejs stream
    }
  });
  ...
  parser.on('endElement', function(name) {
    ...
    if (name.match(recordRegEx)) {
      ...
      try {
        ...
        self.emit('error-record-process', e)
      }
    }
